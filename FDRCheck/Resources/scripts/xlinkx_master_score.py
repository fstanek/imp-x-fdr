...
name_file_xlinkx= input("please copy-paste the whole name of the document with the csv extension as well:")

import os
name_of_the_image = os.path.splitext(name_file_xlinkx)[0]
...
wb = Workbook()
ws = wb.active
with open(name_file_xlinkx, 'r') as f:
...
for i in range(1,counter):
    ...
    string1 = pivot[4]
    string2 = pivot[8]
    protein_name_1 = pivot[2]
    protein_name_2 = pivot[6]
    final_position_1 = int(pivot[3])+int(pivot[5])-1
    final_position_2 = int(pivot[7])+int(pivot[9])-1 
    score = float(pivot[1])
...
def fdr_diagamm(list_crosslinks):
    ...
    plt.savefig(name_of_the_image+"XlinkX_FDR.png")
    plt.clf()
    ...
    plt.legend()
    plt.savefig(name_of_the_image+"XlinkX_FDR2.png")
    ...
    plt.legend()
    plt.savefig(name_of_the_image +"XlinkX_FDR3.png")
