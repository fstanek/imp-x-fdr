...
score_cutoff_interlink_1procent = float(input("pls introduce score_cutoff_interlink_1procent:"))
score_cutoff_interlink_5procent = float(input("pls introduce score_cutoff_interlink_5procent:"))
score_cutoff_intralink_1procent = float(input("pls introduce score_cutoff_intralink_1procent:"))
score_cutoff_intralink_5procent = float(input("pls introduce score_cutoff_intralink_5procent:"))
...
name_file_merox2= input("please copy-paste the whole name of the document with the zhrm extension as well:")
...
merge_all_to_a_book(glob.glob("Result.csv"), "Result.xlsx")
'''
...
for i in range(counter):
    ...
    if site_1_protein_name==site_2_protein_name:
        if float(score_holder)>=score_cutoff_intralink_5procent:
    ...
    elif float(score_holder)>=score_cutoff_interlink_5procent:
            csms.append([string1,string2,position1,position2,site_1_protein_name,site_2_protein_name,site_1_starting_position,site_2_starting_position,total_position_1,total_position_2,pivot[0]])
            csms_clone.append([string1,string2,position1,position2,site_1_protein_name,site_2_protein_name,site_1_starting_position,site_2_starting_position,total_position_1,total_position_2])
...
def fdr_diagamm(list_crosslinks):
        ...
        for j in range(len(list_crosslinks)):
            if list_crosslinks[list_dictionary_of_the_best_score[j]]>=list_of_scores[i]:
                ...
                temp1.append(list([list_dictionary_of_the_best_score[j][0],list_dictionary_of_the_best_score[j][1],
                                   ...
                                   list_dictionary_of_the_best_score[j][8],list_dictionary_of_the_best_score[j][9]]))
                found = False
                k = 0
    ...
    plt.savefig(name_of_the_image + "FDR.png")
    plt.clf()
    ...
    plt.legend()
    plt.savefig(name_of_the_image + "FDR2.png")
    ...
    plt.legend()
    plt.savefig(name_of_the_image + "FDR3.png")
#
#
...
for i in range(good_count):
    ...
    while (j<number_groups and found == False):
        k=0
        while (k<peptides_per_group and found == False ):
